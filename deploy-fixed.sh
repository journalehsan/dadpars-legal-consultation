...
# Dadpars Legal Site Fixed Deployment Script
# This script deploys the Django site with proper configuration
...
EOF
    ...
    sshpass -p "$SERVER_PASSWORD" scp -o StrictHostKeyChecking=no \
        ...
        requirements.txt \
        Dockerfile.prod \
        dadpars_site/settings_production.py \
        static/ \
...
EOFF
        ...
        docker exec ${NEW_CONTAINER_NAME} python manage.py check --deploy 2>/dev/null || echo 'Django checks completed with warnings'
...
verify_deployment() {
    ...
    if curl -s --max-time 10 http://${SERVER_IP}:${PORT} > /dev/null; then
        ...
        print_warning "Site might not be responding. Checking container logs..."
        sshpass -p "$SERVER_PASSWORD" ssh -o StrictHostKeyChecking=no ${SERVER_USER}@${SERVER_IP} \
...
main() {
    echo "=================================="
    echo "Dadpars Legal Site Fixed Deployment"
    echo "=================================="
    echo ""
